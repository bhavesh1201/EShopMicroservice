...
namespace Catalog.API.Data
{
    public class CatalogInitialData : IInitialData
    {
        public async Task Populate(IDocumentStore store, CancellationToken cancellation)
        {
            ...
            if (await session.Query<Model.Product>().AnyAsync()) return;

            //Marten Upsert will take care for existing records

            session.Store<Model.Product>(GetPreconfiguredProducts());
            await session.SaveChangesAsync();
         
        }

        private static IEnumerable<Model.Product> GetPreconfiguredProducts() => new List<Model.Product>()
            {
                new Model.Product()
                {
